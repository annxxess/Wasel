...
export default function CustomerDashboard() {
  ...
  const handleReview = async (e) => {
    ...
    setSubmitting(true);
    try {
      await reviewsAPI.submit({ user_id: user.id, ...review });
  ...
  const totalSpent = orders.filter(o => o.status === 'delivered')
  ...
  return (
    <div className={`min-h-screen ${bg} ${text} transition-colors duration-300`}>
      ...
      {/* Navbar */}
      <nav className={`sticky top-0 z-40 border-b px-4 md:px-8 py-4 flex items-center justify-between backdrop-blur-md ${dark ? 'bg-slate-900/90 border-slate-700' : 'bg-white/90 border-slate-200'}`}>
        <div className="flex items-center gap-3">
          <Link to="/" className="w-9 h-9 bg-gradient-to-br from-blue-600 to-orange-500 rounded-xl flex items-center justify-center text-white font-black text-lg shadow-lg shadow-blue-500/30">
            W
          </Link>
          <div>
          ...
          <div className={`rounded-2xl border overflow-hidden ${card}`}>
            {loading ? (
              ...
              <div className="divide-y divide-slate-700/50">
                {orders.map(o => (
                  <div key={o.id} className={`p-5 hover:${dark ? 'bg-slate-700/30' : 'bg-slate-50'} transition-colors`}>
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        <span className="text-2xl">{o.order_type === 'food' ? '🍔' : o.order_type === 'pharmacy' ? '💊' : '📦'}</span>
                        <div>
